...
def dpal_vit_tiny(patch_size=16, **kwargs):
    model = VisionTransformer(
        ...
        pretrained='/path-to/dpal-vit-t.pth',
        **kwargs)
    return model
...
def vit_small(patch_size=16, **kwargs):
    model = VisionTransformer(
        img_size=(256, 128),
        patch_size=patch_size, embed_dim=384, depth=12, num_heads=6, num_heads_in_last_block=6, mlp_ratio=4,
        qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), 
        pretrained='/path-to/vit-s.pth',
        **kwargs)
    return model
