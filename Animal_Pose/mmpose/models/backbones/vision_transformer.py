...
class ExpertViT(VisionTransformer):
    ...
    def forward(self, base_imgs, meta):
        ...
        #  single-person global local relations diffusion patch
        ...
        for i in range(len_base_inputs):
            outputs['qkv_attens'].append([qk_atten[i], vv_atten[i]])
        # diffusion patch
        outputs['diffusion_patch'] = list(torch.chunk(b_feats, len_base_inputs, dim=0))

        # multi-person local relations
        b_feats, last_atten = self.forward_backbone(meta['crowd_imgs'])
        outputs['feats_from_teacher_local'].append(b_feats[:,1:,:])
        outputs['qkv_attens'].append(last_atten)
        outputs['diffusion_patch'].append(b_feats[:,1:,:])
