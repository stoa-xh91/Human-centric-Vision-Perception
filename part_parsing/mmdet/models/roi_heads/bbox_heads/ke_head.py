...
class KEHead(BBoxHead):
    def __init__(self,
        ...
        # temp for extract
        self.iter = 0
        self.cls_count = nn.Parameter(
                torch.zeros(47), requires_grad=False)
        self.save_feats = nn.Parameter(
                torch.zeros(47, 344, 256), requires_grad=False)
    ...
    def forward(self,
        ...
        # save 
        if self.iter < 45620:
            self.cls_count[pos_gt_labels] += 1
            self.save_feats[pos_gt_labels] += x[:-self.num_classes, :, :].permute(1, 0, 2)
            self.iter += 1
        else:
            temp = self.save_feats / self.cls_count.reshape(-1, 1, 1)
            torch.save(temp, "work_dirs/save_feats.pth")
        # transformer
