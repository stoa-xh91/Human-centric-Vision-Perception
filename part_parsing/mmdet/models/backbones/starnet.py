...
class StarNet(BaseModule):
    def __init__(self, base_dim=32, depths=[3, 3, 12, 5], mlp_ratio=4, drop_path_rate=0.0, num_classes=-1,
        ...
        if init_cfg is not None and init_cfg['type'] =='Pretrained' and init_cfg['checkpoint'] != '':
            self.init_weights()
...
class KDStarNet(StarNet):
    ...
    def forward_features(self, x):
        ...
        for i, blk in enumerate(self.vit_block):
            x = blk(x)
        x = self.final_norm(x)
        outs[-1] = x[:, 1:, :].view(b, h, w, self.embed_dim).permute(0, 3, 1, 2).contiguous()
        return outs
