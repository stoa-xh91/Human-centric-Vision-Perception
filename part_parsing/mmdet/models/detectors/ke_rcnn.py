...
class KERCNN(TwoStageDetector):
    ...
    def simple_test(self, img, img_metas, rescale, **kwargs):
        ...
        # garments_bboxes = kwargs['gt_bboxes'][0][0]
        # garments_scores = torch.ones((garments_bboxes.shape[0], 1), device=garments_bboxes.device)
        # garments_bboxes = torch.cat((garments_bboxes, garments_scores), dim=-1)
        # garments_labels = kwargs['gt_labels'][0][0]
        # garments_scores = F.one_hot(garments_labels)
        # temp = torch.zeros((garments_bboxes.shape[0], 1), device=garments_bboxes.device)
        # garments_scores = torch.cat((garments_scores, temp), dim=-1)
        # attr_results = self.attr_head.simple_test(
        #     x, 
        #     garments_bboxes,
        #     garments_scores,
