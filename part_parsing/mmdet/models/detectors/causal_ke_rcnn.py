...
class CausalKERCNN(TwoStageDetector):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(CausalKERCNN, self).__init__(
            backbone=backbone,
            neck=neck,
            rpn_head=rpn_head,
            roi_head=roi_head,
    ...
    def forward_train(self,
        ...
        if self.use_aug:
            ...
            x = [mlvl_feats[i][:bs] for i in range(num_levels)]
            aug_x = [mlvl_feats[i][bs:] for i in range(num_levels)]
            kwargs['aug_x'] = aug_x
        else:
