...
@PIPELINES.register_module()
class LoadFourierFashionImageFromFile(LoadImageFromFile):
    """Load an image from file.
    ...
    def __init__(self,
                 *arg,
                 ...
                 aug_type=[],
                 **kwargs):
        super(LoadFourierFashionImageFromFile, self).__init__(*arg, **kwargs)
    ...
    def __call__(self, results):
        ...
        if self.use_anime:
            if 'train' in filename:
                aug_filename = filename.replace('train', 'anime'+'_train')
            elif 'test' in filename:
                aug_filename = filename.replace('test', 'anime'+'_test')
        ...
        if self.use_sketch:
            if 'train' in filename:
                aug_filename = filename.replace('train', 'sketch'+'_train')
            elif 'test' in filename:
                aug_filename = filename.replace('test', 'sketch'+'_test')
            aug_img_bytes = self.file_client.get(aug_filename)
            aug_img = mmcv.imfrombytes(
                aug_img_bytes, flag=self.color_type, channel_order=self.channel_order)
            if self.to_float32:
        ...
        aug_filename = filename.replace('train', aug_type+'_train')
