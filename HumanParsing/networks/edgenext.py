...
class EdgeNeXt(nn.Module):
    ...
    def forward(self, x):
        ...
        x = feats[-1].flatten(2).transpose(1, 2).contiguous()
        x = self.norm(x)
        feats[-1] = x.view(b, h, w, c).permute(0, 3, 1, 2).contiguous()
        return feats
...
class EdgeNeXt_SMALL_HP(nn.Module):

    def __init__(
        self,
        context_in_channel=304,
        fusion_in_channel=832,
    ):
        super().__init__()
        
        self.image_encoder = edgenext_small()  #(48, 96, 160, 304)
        
        self.context_encoding = PSPModule(context_in_channel, 512) # tiny & small
        
        self.edge = Edge_Module(in_fea=[48, 96, 160], mid_fea=192, out_fea=2)
       
        self.decoder = Decoder_Module(20, ch1=48, ch2=24, ch3=280) 
...
def edgenext_small_hp(**kwargs):
    model = EdgeNeXt_SMALL_HP() 
