...
class StarNet(nn.Module):
    ...
    def _init_weights(self, m):
        ...
        elif isinstance(m, nn.LayerNorm or nn.BatchNorm2d or nn.BatchNorm1d):
            nn.init.constant_(m.bias, 0)
    ...
    def forward(self, x):
        ...
        # x = self.norm(x)
        b,c,h,w = x.shape
        x = self.norm(x.flatten(2)).view(b,c,h,w).contiguous()
        outs[-1] = x
        return outs
...
class StarNet_S3_HP(nn.Module):
    ...
    def __init__(
        ...
        self.image_encoder = starnet_s3()  #(32, 64, 128, 256)
        
        self.context_encoding = PSPModule(context_in_channel, 512) # tiny & small
        
        self.edge = Edge_Module(in_fea=[32, 64, 128], mid_fea=192, out_fea=2)
       
        self.decoder = Decoder_Module(20, ch1=32, ch2=32, ch3=288) 
