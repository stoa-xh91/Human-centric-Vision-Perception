...
class TransReID(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, 
                ...
                local_feature=False, sie_xishu =1.0, hw_ratio=1, gem_pool = False, stem_conv=False, pretrained=None, 
                frozen_stages=-1, with_cls_token=True,**args):
        super().__init__()
        self.img_size = img_size
    ...
    def _interpolate_pos_encoding(self, x, h, w):
        ...
        dim = x.shape[-1]
        OH = self.img_size[0] // self.patch_embed.patch_size[0]
        OW = self.img_size[1] // self.patch_embed.patch_size[1]
...
def vit_large_patch16_224_TransReID(img_size=(256, 128), stride_size=16, drop_path_rate=0.1, camera=0, view=0,local_feature=False,sie_xishu=1.5, **kwargs):
    print('vit large')
    model = TransReID(img_size=img_size, patch_size=16, stride_size=stride_size, embed_dim=1024, depth=24, num_heads=16, mlp_ratio=4, qkv_bias=True, camera=camera, view=view, drop_path_rate=drop_path_rate, sie_xishu=sie_xishu, local_feature=local_feature, **kwargs)
    model.in_planes = 1024
    return model

def dpal_vit_tiny_patch16_224_TransReID(img_size=(256, 128), stride_size=16, drop_path_rate=0.1, camera=0, view=0, local_feature=False, sie_xishu=1.5, **kwargs):
    print('dpal vit tiny')
    model = KDTransReID(img_size=img_size, patch_size=16, stride_size=stride_size, embed_dim=192, depth=12, num_heads=6, num_heads_in_last_block=12, mlp_ratio=4, qkv_bias=True,drop_path_rate=drop_path_rate, camera=camera, view=view, sie_xishu=sie_xishu, local_feature=local_feature,  frozen_stages=-1, **kwargs)
    return model

def dpal_vit_small_patch16_224_TransReID(img_size=(256, 128), stride_size=16, drop_path_rate=0.1, camera=0, view=0, local_feature=False, sie_xishu=1.5, **kwargs):
    print('dpal vit small')
...
def dpal_vit_base_patch16_224_TransReID(img_size=(256, 128), stride_size=16, drop_path_rate=0.1, camera=0, view=0, local_feature=False, sie_xishu=1.5, **kwargs):
    print('dpal vit base')
    model = KDTransReID(img_size=img_size, patch_size=16, stride_size=stride_size, embed_dim=768, depth=12, num_heads=12, num_heads_in_last_block=16, mlp_ratio=4, qkv_bias=True,drop_path_rate=drop_path_rate, camera=camera, view=view, sie_xishu=sie_xishu, local_feature=local_feature,  **kwargs)
    model.in_planes = 768
    return model
