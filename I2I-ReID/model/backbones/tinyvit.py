...
class KDTinyViT(TinyViT):
    def __init__(self, img_size=[224,224], in_chans=3,
        ...
        for i_layer in range(self.num_layers):
            kwargs = dict(dim=embed_dims[i_layer],
                        input_resolution=(patches_resolution[0]// (2 ** (i_layer if i_layer <3 else 2)),
                                    patches_resolution[1]//(2 ** (i_layer if i_layer <3 else 2))),
                          depth=depths[i_layer],
                          drop_path=dpr[sum(depths[:i_layer]):sum(depths[:i_layer + 1])],
                          downsample=PatchMerging if (
                              i_layer < self.num_layers - 1) else None,
        ...
        self.adapter = nn.Linear(192, 1024)
