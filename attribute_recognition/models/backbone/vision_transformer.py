...
class VisionTransformer(nn.Module):
    """ Vision Transformer """
    def __init__(self, pretrained=None,img_size=(256, 192), patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
                 num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., norm_layer=nn.LayerNorm, frozen_stages=-1, **kwargs):
        super().__init__()
    ...
    def init_weights(self, pretrained=None):
        if pretrained == '' or not pretrained:
            return 
    ...
    def forward(self, x):
        # print('begin:',x.shape)          [64, 3, 256, 128]
        # print(x.shape)
        x = self.prepare_tokens(x)
        # print(x.shape)
        for i,blk in enumerate(self.blocks):
            x = blk(x)
        x = self.norm(x)   #[64, 129, 192]
...
class kd_vit(VisionTransformer):
    """ Vision Transformer """
    def __init__(self, pretrained=None,img_size=(256, 192), patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
                 num_heads=6, num_heads_in_last_block=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., norm_layer=nn.LayerNorm, **kwargs):
        super().__init__(pretrained, img_size, patch_size, in_chans, num_classes, embed_dim, depth,
...
class align_vit(VisionTransformer):
    """ Vision Transformer """
    def __init__(self, pretrained=None,img_size=(256, 192), patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
                 num_heads=6, num_heads_in_last_block=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., norm_layer=nn.LayerNorm, **kwargs):
...
@BACKBONE.register("vit_tiny")
def vit_tiny(patch_size=16, **kwargs):
    model = VisionTransformer(
        pretrained='/raid/wangxuanhan/models_saipv2/lup1m_ViTKD_path-b_to_vit_tiny/checkpoint.pth',
        patch_size=patch_size, embed_dim=192, depth=12, num_heads=3, mlp_ratio=4,
        qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    print('Load ViT Done !')
    return model

@BACKBONE.register("kd_vit_tiny")
def kd_vit_tiny(patch_size=16, **kwargs):
    model = kd_vit(
        pretrained='/raid/wangxuanhan/models_saipv2/lup1m_maskedkd_path-b_to_vit_tiny/checkpoint.pth',
        patch_size=patch_size, embed_dim=192, depth=12, num_heads=6, num_heads_in_last_block=12, mlp_ratio=4,
        qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    # model = align_vit(
    #     pretrained='/raid/wangxuanhan/research/downstream_tasks/pose_estimation/mmpose-main/models/lup1m_repa_align_path_pre_vit_tiny.pth',
    #     img_size=(256, 128),
    #     patch_size=patch_size, embed_dim=192, depth=12, num_heads=3, num_heads_in_last_block=3, mlp_ratio=4,
    #     qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), frozen_stages=-1, **kwargs)
    print('Load KD ViT Done !')
    return model

@BACKBONE.register("kd_vit_small")
def kd_vit_small(patch_size=16, **kwargs):
    model = VisionTransformer(
        pretrained='/raid/wangxuanhan/models_saipv2/lup1m_path-b_to_vit_small_from_cls_patch_moe/checkpoint0100.pth',
        patch_size=patch_size, embed_dim=384, depth=12, num_heads=6, num_heads_in_last_block=12, mlp_ratio=4,
        qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    print('Load KD ViT-Small Done !')
    return model

@BACKBONE.register("vit_small")
def vit_small(patch_size=16, **kwargs):
    model = VisionTransformer(
        pretrained='/raid/wangxuanhan/research/downstream_tasks/pose_estimation/mmpose-main/models/lupsub_csl_pre_sep_vit_small.pth',
        patch_size=patch_size, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4,
        qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    print('Load ViT-Small Done !')
    return model

@BACKBONE.register("vit_base")
def vit_base(patch_size=16, **kwargs):
    # model = VisionTransformer(
    #     pretrained='/raid/wangxuanhan/research/downstream_tasks/pose_estimation/mmpose-main/models/lup_hap_pre_vit_base.pth',
    #     img_size=(224, 224),
    #     patch_size=patch_size, embed_dim=768, depth=12, num_heads=12, num_heads_in_last_block=16, mlp_ratio=4,
    #     qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), frozen_stages=-1, **kwargs)
    model = kd_vit(
        pretrained='/raid/wangxuanhan/models_saipv2/lup1m_path-l_to_vit_base_from_cls_patch_atten_moe/checkpoint.pth',
        img_size=(256, 128),
        patch_size=patch_size, embed_dim=768, depth=12, num_heads=12, num_heads_in_last_block=16, mlp_ratio=4,
        qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), frozen_stages=-1, **kwargs)
    print('Load ViT-Base Done !')
    return model